...
public class DataUtility {
     ...
     * @return A combined list containing the average of trend and seasonality predictions.
     */
    public static ArrayList<Double> combine(ArrayList<Double> trendPrediction,
        ...
        int size = Math.min(trendPrediction.size(), seasonalityPrediction.size());
        
        for (int i = 0; i < size; i++) {
            double trend = trendPrediction.get(i);
            double seasonality = seasonalityPrediction.get(i);
            
            // Simple Average Ensemble
            double avg = (trend + seasonality) / 2.0;
            combined.add(avg);
        }
        
        // If sizes differ, we stop at the minimum size to avoid index errors.
        // The original code tried to set elements in seasonalityPrediction which implies
        // they were expected to be of same size or seasonality was larger.
